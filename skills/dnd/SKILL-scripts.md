...
**Play-session binding.** "A campaign is loaded" is equally true in a session spent editing this repo, so the hook also checks *which* session is playing. `/dm:dnd load` rewrites `active-campaign.json` without a `session_id`; the first Stop hook to run after that claims the campaign by merging its own `session_id` into the marker. Every other session then no-ops. Without this, a dev session gets continuity-flush prompts aimed at a live campaign, and its turns are linted as if they were DM narration — on 2026-07-20 the first entry ever written to a lint log was a false positive produced by a dev session that merely *quoted* a roll request. A manual `--campaign` run is an explicit override and skips the gate. Every turn it snapshots `state.md` to the runtime dir; every N turns (default 10, `DND_AUTOSAVE_EVERY` to override) it emits a Stop-hook `block` decision that prompts the DM to flush continuity before yielding. The hook is **opt-in** — the in-model micro-save cadence works without it.
