...
3. **Backs up and retires** the old `~/.claude/skills/dnd` (moved to
...
python3 migrate_v1_to_v2.py --keep-standalone   # verify only; leave /dnd in place
```
...
discussion thread on the repo. If something didn't carry over cleanly, please
drop a note there with your prior version and OS so we can catch it early.
